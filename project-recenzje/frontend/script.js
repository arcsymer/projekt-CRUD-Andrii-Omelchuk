...
const polaObowiazkowe = [formularz.danie, formularz.restauracja, formularz.adres, formularz.data, formularz.ocena, formularz.rekomendacja];
...
formularz.addEventListener('submit', async e => {
    ...
    const dane = {
        ...
        rekomendacja: formularz.rekomendacja.value,
        spożycie: formularz.spożycie.value.trim() || null,
...
window.edytujRecenzje = r => {
    ...
    formularz.rekomendacja.value = r.rekomendacja;
    formularz.spożycie.value = r.spożycie || '';
    ...
    try {
        ...
        data.ostatnie.forEach(r => {
            ...
            if (r.rekomendacja) {
                ...
                rek.textContent = `Rekomendacja: ${r.rekomendacja}`;
                li.appendChild(rek);
                li.appendChild(document.createElement('br'));
            }
            if (r.spożycie) {
                const sp = document.createElement('span');
                sp.textContent = `Z czym: ${r.spożycie}`;
                li.appendChild(sp);
                li.appendChild(document.createElement('br'));
            }
            if (r.komentarz) {
