document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (document.getElementById('most-visited-list')) {
    ...
    if (document.getElementById('add-review-btn') || document.getElementById('edit-review-btn')) {
        const btn = document.getElementById('add-review-btn') || document.getElementById('edit-review-btn');
        const fields = ['dish-name','restaurant-name','restaurant-address','visit-date','rating','recommendation','with-what','comment'].map(id => document.getElementById(id));
        enableButtonOnFields(btn, fields.slice(0,6));
        ...
        if (document.getElementById('edit-review-btn')) {
            ...
            fields.forEach(f => f.value = review[f.id]);
        }
    }
    ...
    if(document.getElementById('weather')) {
        ...
        const API_KEY = 'fd66f06221a276aa0235b98f0e57797d';
        async function loadWeather() {
            try {
                const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=Warsaw,PL&appid=${API_KEY}&units=metric`);
                const data = await res.json();
                if(data.main) {
                    const temp = data.main.temp.toFixed(1);
                    const desc = data.weather[0].description;
                    let color = 'blue';
                    if(temp >= 25) color = 'orange';
                    else if(temp >= 15) color = 'goldenrod';
                    else if(temp >= 5) color = 'lightblue';
                    else color = 'cyan';
                    weatherDiv.innerHTML = `🌤 ${temp}°C, ${desc}`;
